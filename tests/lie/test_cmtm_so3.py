...
def test_cmtm_so3_vec3d_ptan_map():
    ...
    np.testing.assert_allclose(res.ptan_map(), mat, rtol=1e-10, atol=1e-10)

def test_cmtm_so3_ptan_map_ptan_vec():
    for i in range(2,test_order):
        res = mr.CMTM.rand(mr.SO3,i)

        vec = res.ptan_map(i-1) @ res.vecs_flatten()

        np.testing.assert_allclose(vec, res.ptan_vecs_flatten(), rtol=1e-10, atol=1e-10)
...
def test_cmtm_so3_ptan_map_inv_ptan_vec():
    for i in range(2,test_order):
        ...
        np.testing.assert_allclose(vec, res.vecs_flatten(), rtol=1e-10, atol=1e-10)
...
def test_cmtm_so3_ptan_inv():
    for i in range(test_order):
        ...
        np.testing.assert_allclose(res.ptan_map() @ res.ptan_map_inv(), mat, rtol=1e-8, atol=1e-8)
        np.testing.assert_allclose(res.ptan_map_inv() @ res.ptan_map(), mat, rtol=1e-8, atol=1e-8)
...
def test_cmtm_so3_ptan_to_tan():
    for i in range(test_order):
        ...
        for j in range(i):
            np.testing.assert_allclose(res.ptan_to_tan(3, i)[3*j:3*(j+1),3*j:3*(j+1)], mat, rtol=1e-10, atol=1e-10)
            mat = mat / (j+1)
...
def test_cmtm_so3_tan_to_ptan():
    for i in range(test_order):
        ...
        for j in range(i):
            np.testing.assert_allclose(res.tan_to_ptan(3, i)[3*j:3*(j+1),3*j:3*(j+1)], mat, rtol=1e-10, atol=1e-10)
            mat = mat * (j+1)

def test_cmtm_so3_tan_to_ptan_inv():
    for i in range(test_order):
        so3 = mr.SO3.rand()
        vel = np.random.rand(i,3)
        ...
        np.testing.assert_allclose(res.tan_to_ptan(3, i) @ res.ptan_to_tan(3, i), np.eye(3*(i)), rtol=1e-10, atol=1e-10)
        np.testing.assert_allclose(res.ptan_to_tan(3, i) @ res.tan_to_ptan(3, i), np.eye(3*(i)), rtol=1e-10, atol=1e-10)
...
def test_cmtm_so3_tan_map_tan_vec():
    for i in range(2,test_order):
        ...
        np.testing.assert_allclose(vec, res.tan_vecs_flatten(), rtol=1e-10, atol=1e-10)

def test_cmtm_so3_tan_map_inv_tan_vec():
    for i in range(2,test_order):
        res = mr.CMTM.rand(mr.SO3,i)
...
def test_cmtm_so3_tan_inv():
    ...
    for i in range(test_order):
        ...
        np.testing.assert_allclose(res.tan_map() @ res.tan_map_inv(), mat, rtol=1e-10, atol=1e-10)
        np.testing.assert_allclose(res.tan_map_inv() @ res.tan_map(), mat, rtol=1e-10, atol=1e-10)
...
def test_cmtm_so3_sub():
    for i in range(test_order):
        ...
        np.testing.assert_allclose(vec1, vec2, rtol=1e-10, atol=1e-10)
...
def test_cmtm_so3_sub_tan_vec():
    for i in range(test_order):
        ...
        np.testing.assert_allclose(res, vec, rtol=1e-10, atol=1e-10)
