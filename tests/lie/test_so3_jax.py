...
def test_so3():
  ...
  np.testing.assert_array_equal(res.mat(), jnp.eye(3))
  
def test_so3_inv():
  rot = mr.SO3.rand(LIB='jax')
  res = rot @ rot.inv()
  
  np.testing.assert_allclose(res.mat(), jnp.identity(3), rtol=1e-15, atol=1e-15)
...
def test_so3_hat():
  v = np.random.rand(3)  
  m = jnp.array([[0., -v[2], v[1]],[v[2], 0., -v[0]],[-v[1], v[0], 0.]])
  
  res = mr.SO3.hat(v, LIB='jax')
...
def test_so3_hat_commute():
  ...
  res1 = mr.SO3.hat(v1, LIB='jax') @ v2
  res2 = mr.SO3.hat_commute(v2, LIB='jax') @ v1
  
  np.testing.assert_allclose(res1, res2, rtol=1e-8, atol=1e-8)
...
def test_so3_matmul():  
  ...
  np.testing.assert_allclose(res.mat(), jnp.eye(3), rtol=1e-15, atol=1e-15)
...
def test_so3_matmul_mat():
  ...
  np.testing.assert_allclose(res, jnp.eye(3), rtol=1e-15, atol=1e-15)
