...
def test_factorial_vector():
    ...
    assert factorial_vector.vec_factorial().shape == (test_dim*test_order,)
    ...
    np.testing.assert_allclose(factorial_vector.vec_factorial(), expected.flatten())

def test_factorial_matrix():
    vec = np.random.rand(test_order)
    factorial_vector = mr.FactorialVector(vec)
    factorial_mat = factorial_vector.factorial_mat()
    inverse_factorial_mat = factorial_vector.inverse_factorial_mat()
